...
async def uart_init():
    ...
    while True:
        if _UART.any(): # Check any bytes in buffer
            line = _UART.readline()
            # If reading is not None, at least 2 long (2nd byte is HR) and HR is realistic
            valid = True
            if line is None: valid = False
            elif len(line) < 2: valid = False 
            elif line[0] != 16: valid = False
            elif int(line[1]) >= 210 or int(line[1]) <= 30: valid = False
            
            if valid:
                _CURRENT_HR = line
                try:
                    print("uart data received: ", line, " = ", line[1], " bpm")                
                except Exception as e:
                    print("ERROR: ", e)
            else:
                try:
                    print("invalid data received: ", line, " . Resend last data point ", str(_CURRENT_HR), " bpm")                
                except Exception as e:
                    print("ERROR: ", e)
